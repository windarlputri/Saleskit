...
?>
	...
	<div class="form-group required">
		...
		<div class="col-md-7">
			<select id="cabang" name="cabang" class="form-control ipts select2" required="" style="width: 100%" data-placeholder="Pilih Cabang">
				<option></option>
				<?php
				if(!empty($cabang))
				{
					...
					{
						$ck='';
						if(trim($r->Branch)==$row->cabang)
						{
							$ck='selected="selected"';
						}
						echo '<option value="'.trim($r->Branch).'" '.$ck.'>'.trim($r->Branch).' - '.trim($r->Name).'</option>';
					}
				}
				?>
			</select>
