...
class Users extends H_Controller
{
    ...
    function viewdata()
    {
		...
		$this->datatables->select("u.ID as ii,u.username as username,u.full_name as name,r.role_value as role,u.status as status,u.email as email, u.cabang + ' - ' + c.Name as cabang")
		    ->unset_column('ii')
		    ->edit_column('action',$this->button('$1'),"ii")
		    ->from('users u
		    LEFT JOIN user_role r ON (u.user_role_id=r.ID)
		    LEFT JOIN KODE_CABANG c ON (u.cabang=c.Branch)
		    ')
		    ->where($s);
	...
	function add()
	{
		$d['role']=$this->user_roles->roles_data(array('is_add'=>1));
    	$d['status']=$this->user_model->user_status();
    	$d['cabang']=$this->user_model->user_cabang();
    	$d['url']=$this->mod_url;
	...
	function addajax()
	{
		if($this->input->is_ajax_request()==TRUE)
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($p1==$p2)
				{
					$act=$this->user_model->user_add($role,$username,$p1,$email,$full_name,$nick_name,$avatar,$status,$cabang);
	...
	function editajax()
	{
		if($this->input->is_ajax_request()==TRUE)
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($n==TRUE)
				{
					$act=$this->user_model->user_edit($userID,$role,$username,$p1,$email,$full_name,$nick_name,$avatar,$status,$cabang);
					rb_message_header_set($act['status'],'Add User',$act['message']);
					echo json_encode(array(
						'status'=>$act['status'],
						'message'=>$act['message'],
